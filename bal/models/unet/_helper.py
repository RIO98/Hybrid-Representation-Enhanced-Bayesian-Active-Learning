...
from bal.models.links.noise import MCDropout
from bal.models.functions import stride_pooling_nd
from ..initializers import bilinear_upsample
from bal.models.links.connection import PixelShuffleUpsampler2D
from bal.models.links.connection import PixelShuffleUpsampler3D
...
def norm(ndim, size, param):
    ...
    if param['name'] == 'group':
        num_group = 32
        while size / num_group != size // num_group:
            num_group //= 2

        assert size / num_group == size // num_group, f"{size}, {num_group}"
        param['num_groups'] = num_group
        param['num_channels'] = size
    else:
